...
internal class ExplosionDamage : Effect<DamagedEvent>
{
    ...
    public ExplosionDamage(ISampleProvider source, Audio audio) : base(source, audio)
    {
    }
    ...
    {
        UpdateVolume(s.ExplosionDamage);
        _freq           = s.ExplosionDamage.Frequency;
        _attackEnd      = s.ExplosionDamage.AttackEnd  / 100f;
        _decayStart     = s.ExplosionDamage.DecayStart / 100f;
        _maxLength      = s.ExplosionDamage.MaxLength  / 1000f;
        _ampExponent    = s.ExplosionDamage.AmplitudeExponent;
        _lengthExponent = s.ExplosionDamage.LengthExponent;
    }
}